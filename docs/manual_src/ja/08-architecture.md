...
spawn された子プロセス（`spawn`、`system`）の場合、子は `RUBYOPT=-rrperf` と環境変数（`RPERF_SESSION_DIR`、`RPERF_ROOT_PROCESS`）を継承します。子の Ruby プロセスが rperf を読み込むと、オートスタートブロックがルートプロセスでないことを検出し、プロファイルをセッションディレクトリに直接書き込みます。
